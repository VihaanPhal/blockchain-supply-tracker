...
contract AccessControlManager is AccessControl {
    ...
    function revokeRetailer(address account) external onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(RETAILER_ROLE, account);
    }

    function revokeRegulator(address account) external onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(REGULATOR_ROLE, account);
    }
